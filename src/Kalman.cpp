...
namespace Kalman {
    ...
    Filter::Filter(double odom_x, double odom_y) : state(odom_x, odom_y) {
        ...
        noise = 0.00025 * Eigen::Matrix2d::Identity(); // odom drift
    }

    Pose Filter::getEstimation() const {
        return {state(0), state(1), heading};
    }
    ...
    void Filter::predict(double prior_odom_x, double prior_odom_y, double odom_x,
                         double odom_y, double imu_heading) {
                            ...
                            // displacement
                            double travel_dist = std::sqrt((travel_dist_x * travel_dist_x) + (travel_dist_y * travel_dist_y));

                            // using imu heading
                            double predicted_x = travel_dist * std::cos(imu_heading);
                            double predicted_y = travel_dist * std::sin(imu_heading);

                            // adjust current state to be altered by predicted travel dist based on odom wheels + imu
    ...
    void Filter::update(double imu_heading, double front_sens, 
                        double left_sens, double right_sens) {
                            ...
                            for (const auto& [sensor_dist, loc] : sensors) {
                                ...
                                if (std::cos(theta) > 0.0001 || std::cos(theta) < -0.0001) {
                                    ...
                                    if (dist_x < 0) {
                                        dist_x = 1e9; // default back to large num because it needs to be positive
                                    }
                                }
                                ...
                                double variance = (0.01 + (sensor_dist * 0.01)) * (0.01 + (sensor_dist * 0.01));
                                ...
                                Eigen::Matrix<double, 2, 1> kalman_gain = cov * obs.transpose() / uncertainty;
