...
*/
MCL::MCL(double w, double h, double xO, double yO, size_t minP, size_t maxP, double xyN, double t, double s) : 
    mapWidth(w), mapHeight(h), odom{xO, yO, 0}, minParticles(minP), maxParticles(maxP), maxXYNoise(xyN), threshold(t), sigma(s) {   }   // Takes width and height of map
                                                                                                                // and the default position

double MCL::Particle::getDistance(uint8_t mapWidth, uint8_t mapHeight, int s) {
    double angle = theta + (s * (std::numbers::pi / 2)); // we need to represent the sensors on
                                                         // each side
    ...
    const double epsilon = 1e-9;                // smallest representable value without getting outlier values (i.e, inf, -inf, NaN)

    double distanceX, distanceY;

    // need to ensure that no unpredicable values come through. if a ray is completely horizontal, the cos func
    // will give a nice good value, but sin can give weird values, especially as it gets closer to +-pi

    if (std::abs(cos_val) < epsilon) {          // need to clamp distance to "predictable" value
    ...
    if (std::abs(sin_val) < epsilon) {          // need to clamp distance to "predictable" value
...
void MCL::assignWeight(MCL::Particle& p) {
    double diffLeft = distSensors.left - p.getDistance(mapWidth, mapHeight, Sensor::Left);
    double diffFront = distSensors.front - p.getDistance(mapWidth, mapHeight, Sensor::Front);
    double diffRight = distSensors.right - p.getDistance(mapWidth, mapHeight, Sensor::Right);
    double weightLeft = std::exp((-(diffLeft*diffLeft)/(2*sigma*sigma)));
    double weightFront = std::exp((-(diffFront*diffFront)/(2*sigma*sigma)));
    double weightRight = std::exp((-(diffRight*diffRight)/(2*sigma*sigma)));
...
void MCL::resampleParticles() {
    ...
    std::discrete_distribution<size_t> dd(weights.begin(), weights.end());      // create distribution by weights

    // push particles that have the higher probability (weight) of getting chosen into new vector
    for (size_t i = 0; i < maxParticles; i++) {
...
Pose MCL::getEstimation() {
    ...
    for (Particle p : particles) {
        if (p.weight > bestParticle.weight) { bestParticle = p; }
    }
