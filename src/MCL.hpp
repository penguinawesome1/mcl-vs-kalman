...
class MCL {
  ...
  void assignWeight(Particle& p);         // Assigns weight based off difference from actual reading

public:
  MCL(double w, double h, double xO, double yO, size_t minP, size_t maxP, double xyN, double t, double s);                  // Constructor
  ~MCL() = default;                 // Destructor

  void spawnParticles();       // Uniformly distributes particles throughout the map
  void propogateParticles();   // Updates all the particles according to measured movement delta and heading
  void resampleParticles();    // Assigns weights to particles and discards far off particles for optimizations

  void updateOdom(Pose p) { lastOdom = odom; odom = p; }
